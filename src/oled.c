...
void oled_blink1(char left, char right, char width,char count)
 {  char i=left;
    ...
    count++;
     ...
     while(count)
     {
      set_cursor(address_iic_oled, i,y );
      ff_string(address_iic_oled,width);
      set_cursor(address_iic_oled, i+width-1,y );
      blankScreen(address_iic_oled,1);
      set_cursor(address_iic_oled, i,y );
      blankScreen(address_iic_oled,1);
...
char oled_print_mega_char(char c,char x,char y)
{
	char i;
  const char cc=3;
  const char const_ch[7][8]={{0x00,0x38,0x00,0x00, 0x00,0x0e,0x00,0x00},  //0   ':'
                             ...
                             {0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00},  //4   ' '
														 {0x00,0x00,0x00,0x00, 0x60,0x60,0x60,0x60},  //5   '_'
														 {0x06,0x06,0x06,0x06, 0x00,0x00,0x00,0x00},   //6   '~'
                             //{0x00,0x00,0x06,0x00, 0x00,0x00,0x7f,0x00}  //7   'i'
														 //{0x3f,0x3f,0x33,0x3f, 0xff,0x00,0x00,0x00}  //8   'p'
														 //{0x06,0x06,0x06,0x06, 0x00,0x00,0x00,0x00}   //9   'd'														 
                              }; 
  char width_limit;
  char ch[8];
  switch(c)
  ...
  case ' ':{c=4;width_limit=4; break;}
	case '_':{c=5;width_limit=4; break;}
	case '~':{c=6;width_limit=4; break;}
	//case 'i':{c=7;width_limit=4; break;}
	//case 'p':{c=8;width_limit=4; break;}
	//case 'd':{c=9;width_limit=4; break;}
  }
...
switch(font)
  {
   case MEGA:{
      if(n<0) {/*oled_print_mega_char('-',pos,y);pos+=12;*/n=0-n;}
			//else {oled_print_mega_char(' ',pos,y);pos+=12;}
